...
import type { PgnParseResult } from "@/lib/pgn";
...
export function PgnUploadForm({ className }: PgnUploadFormProps) {
  ...
  const handleParse = useCallback((content: string, name?: string) => {
    setIsParsing(true);
  ...
  const onSubmitPaste = useCallback(() => {
    if (pgnText.trim()) {
      handleParse(pgnText.trim());
    }
  }, [handleParse, pgnText]);

  const openOnBoard = useCallback(() => {
    if (!parseResult || parseResult.games.length === 0) {
      return;
    }
    saveStudy({
      fileName,
  ...
  return (
    <div className={className}>
        ...
        {showPaste ? (
          <div className="mt-3">
            ...
            <button
              ...
              disabled={!pgnText.trim() || isParsing}
      ...
      {hasSuccess && firstGame && firstStats ? (
        <div className="mt-6 rounded-xl bg-white p-5 ring-1 ring-zinc-200">
          ...
          <button
            type="button"
            onClick={openOnBoard}
            className="mt-4 w-full rounded-lg bg-green-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-green-800"
          >
            Open on board
          </button>
        </div>
