  ...
  if (moves.length === 0) {
    return (
      <div className="w-full min-w-0">
        <p className="mb-1.5 text-xs font-medium text-muted-foreground">Current line</p>
        <div className="rounded-lg bg-background px-3 py-2.5 ring-1 ring-border">
          <p className="text-sm text-muted-foreground">Starting position</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="w-full min-w-0">
      <div className="mb-1.5 flex items-center justify-between gap-2">
        <p className="text-xs font-medium text-muted-foreground">Current line</p>
        <p className="shrink-0 text-xs text-muted-foreground/80">
          {moves.length} {moves.length === 1 ? "move" : "moves"}
        </p>
      </div>
      <div className="max-h-32 overflow-y-auto overflow-x-hidden rounded-lg bg-background p-2 ring-1 ring-border">
              ...
              <button
                ...
                className={`max-w-full truncate rounded-md px-2 py-1 font-mono text-xs transition ${
                  isActive
                    ...
                    : node.isVariation
                      ? "bg-warning-muted text-warning-foreground hover:bg-warning-muted"
