...
function WeakLinesList({
  ...
  return (
    <ul className="space-y-2">
      {weakLines.map((line) => {
        const mastery = getMastery(line.lineId);
        const hasFailurePly = mastery?.failedAtPly !== undefined;
        return (
          <li key={line.lineId}>
            <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
              <Link
                href={`/training/${repertoireId}?weak=${line.lineId}`}
                className="flex min-w-0 flex-1 items-center justify-between gap-2 rounded-lg border border-border bg-background px-3 py-2 text-sm transition hover:border-accent hover:bg-accent-muted"
              >
                <span className="min-w-0 truncate font-medium text-foreground">
              ...
              {hasFailurePly ? (
                <Link
                  href={`/training/${repertoireId}?weak=${line.lineId}&drill=failure`}
                  className="shrink-0 rounded-lg border border-danger/40 bg-danger-muted px-3 py-2 text-center text-xs font-semibold text-danger transition hover:bg-danger-muted/80"
                >
                  Drill mistake
                </Link>
              ) : null}
